...
/**
 * Full app - demo + fertilizer + advisory
...
export default function App(){
  ...
  async function runDemo(){
    ...
    try{
      ...
      setResp(j ?? null);
    } catch(err){ setResp(null); setError(err.message || "Unknown error"); }
  ...
  async function runFertilizerCalc(e){
    ...
    try{
      ...
      const j = await r.json();
      setFertResult(j);
    } catch(err){ setFertError(err.message || "Unknown error"); }
  ...
  async function runAdvisory(e){
    ...
    try{
      ...
      const j = await r.json();
      setAdvResult(j);
    } catch(err){ setAdvResult({ error: err.message || "Unknown error" }); }
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] text-white flex flex-col">
      ...
      <main className="max-w-4xl mx-auto p-6 flex-grow">
        <h1 className="text-4xl font-extrabold mb-6">Agro-Mitra — Demo</h1>
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <button onClick={() => downloadJSON()} className="bg-gray-700 hover:bg-gray-600 px-3 py-2 rounded-md text-white">Download JSON</button>
        </div>
          ...
          {resp && (
            <>
                <div className="bg-black/30 p-5 rounded-lg border border-white/10 mt-4">
                  ...
                  <div>Temp: <span className="font-medium">{resp.weather?.temp ?? "N/A"} °C</span></div>
                  <div>Humidity: <span className="font-medium">{resp.weather?.humidity ?? "N/A"}%</span></div>
                  <div>Desc: <span className="font-medium">{resp.weather?.description ?? "N/A"}</span></div>
                </div>

              <div className="bg-gray-800 p-4 rounded shadow">
                <h2 className="text-xl font-semibold mb-3">Recommended Crops</h2>
                {Array.isArray(resp.crops) && resp.crops.length ? resp.crops.map((c,i)=>(<div key={i} className="pb-3 mb-3 border-b border-gray-700">
                ...
                </div>)) : <div>No crops</div>}
              </div>

              <div className="bg-gray-800 p-4 rounded shadow">
                <h2 className="text-xl font-semibold mb-3">Fertilizer Plan</h2>
                {Array.isArray(resp.fertilizer_plan) && resp.fertilizer_plan.length ? resp.fertilizer_plan.map((f,i)=>(<div key={i} className="pb-2 mb-2 border-b border-gray-700">
        ...
        <div className="bg-gray-800 p-4 rounded shadow mb-6">
          ...
          <form onSubmit={runFertilizerCalc} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="sm:col-span-3 mt-2">
              <button type="submit" disabled={fertLoading} className="bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded-md text-white">{fertLoading ? "Calculating..." : "Calculate Fertilizer"}</button>
            </div>
          </form>
        ...
        <div className="bg-gray-800 p-4 rounded shadow mb-12">
          ...
          <form onSubmit={runAdvisory} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2">
              <button className="bg-yellow-600 hover:bg-yellow-700 px-4 py-2 rounded-md text-black" disabled={advLoading}>{advLoading ? "Generating..." : "Get Localized Advisory"}</button>
            </div>
          </form>
